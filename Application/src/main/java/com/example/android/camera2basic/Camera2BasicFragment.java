...
public class Camera2BasicFragment extends Fragment
    ...
    private final ImageReader.OnImageAvailableListener mOnImageAvailableListener
        ...
        public void onImageAvailable(ImageReader reader) {
            ...
            image.close();
            enableFlash = false;
            mBackgroundHandler.post(new ImageSaver(bytes, mFile, imageCallBack));
        }
    ...
    private ImageReader.OnImageAvailableListener streamListener = new ImageReader.OnImageAvailableListener() {
        @Override
        public void onImageAvailable(ImageReader reader) {
            Image image = reader.acquireNextImage();
            image.close();
    ...
    private CameraCaptureSession.CaptureCallback mCaptureCallback
        ...
        private void process(CaptureResult result) {
            ...
            Log.d(TAG, "enableFlash " + enableFlash);
            switch (mState) {
                case STATE_PREVIEW: {
...
//                        }
                ...
                case STATE_WAITING_PRECAPTURE: {
                    // CONTROL_AE_STATE can be null on some devices
                    Integer aeState = result.get(CaptureResult.CONTROL_AE_STATE);
                    Log.d(TAG, "WAITING_PRECAPTURE " + aeState);
                    if (aeState == null ||
...
//            process(partialResult);
    ...
    private void updateCachedAECaptureStatus(TotalCaptureResult result) {
        Integer aeState = result.get(CaptureResult.CONTROL_AE_STATE);
        if (aeState != null && aeState == CaptureRequest.CONTROL_AE_STATE_FLASH_REQUIRED) {
            enableFlash = true;
        }
    }
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                imageStreamReader = ImageReader.newInstance(largest.getWidth(), largest.getHeight(),
                        ImageFormat.JPEG, /*maxImages*/2);
                imageStreamReader.setOnImageAvailableListener(
                        streamListener, mBackgroundHandler);
    ...
    private void createCameraPreviewSession() {
        try {
            ...
            mPreviewRequestBuilder.addTarget(imageStreamReader.getSurface());

            // Here, we create a CameraCaptureSession for camera preview.
            mCameraDevice.createCaptureSession(Arrays.asList(surface, imageStreamReader.getSurface(), mImageReader.getSurface()),
...
//            mPreviewRequestBuilder.set(CaptureRequest.FLASH_MODE, CameraMetadata.FLASH_MODE_TORCH);
     ...
     */
    public Object backgroundCamera = new Object();
...
//            setAutoFlash(mPreviewRequestBuilder);
    ...
    private static class ImageSaver implements Runnable {
        ...
        ImageSaver(byte[] image, File file, ImageCallBack callBack) {
            mImage = image;
            mFile = file;
